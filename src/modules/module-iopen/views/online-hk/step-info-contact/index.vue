<template>
  <op-wrap :isDisabled="isDisabled" :handleBefore="handleBefore" @handleNext="handleNext">
    <div class="olhk-step olhk-step-info-contact">
        ...
        <cube-form-group class="step-content custom-form-group border-bottom-1px">
          ...
          <template v-if="isContactAddress">
            <cube-form-item
              ...
              v-if="model.contactRadio !== radioListValue.oth"
            >
              <!-- 大陆省市区 -->
      ...
      <ul class="step-content step-content-address-img-upload">
        <li>
          <camera
            :uploadFiles="AddressImgUploadFiles"
            :uploadList="AddressImgFile"
            :uploadBase="AddressImgUpload"
...
export default {
  ...
  methods: {
    ...
    handleUpload(file, idFlag, callback) {
      ...
      // 判断文件类型是否银行卡
      this.$set(this.AddressImgFile, idFlag, "");
      this.$set(this.AddressImgUpload, idFlag, file);
      this.saveImage(params).then((res) => {
        const { imgUrl } = res;
        // 判断文件类型是否银行卡
        this.$set(this.AddressImgFile, idFlag, imgUrl);
      });
    },
    // 住址地址选择地区
    changeHomeArea(args) {
    ...
    //   if (age < 45) {
    //     this.fieldsProfession.professionCode.props.options = optionsList.industryRangeOptions().filter(
    //       (item) => item.value !== 5
    //     );
    //   }
    // },
    ...
    initData() {
      ...
      const tempKey = `${Address}${SUFFIX}`;
      const tempVal = userImg[tempKey] || "";
      this.$set(this.AddressImgFile, tempKey, tempVal);
      this.$set(this.AddressImgUpload, tempKey, tempVal);
    },
    handleBefore() {
