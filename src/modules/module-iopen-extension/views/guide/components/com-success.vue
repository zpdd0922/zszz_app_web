<template>
  <div class="op-guide-com op-guide-com-success">
    <section class="content">
      <div class="box">
        <em class="status"></em>
        <p class="result-title">{{$t("iopenExt.guide.success.title")}}</p>
        <!-- 开户方式为香港 -->
        <!-- 开户方式为大陆 -->
        <!-- 用户是否入金 -->
      </div>
      <div v-if="openType!=''" class="desc">
        <h4 v-if="openType=='cn'" class="desc-title">
          {{$t("iopenExt.guide.success.desc.tradeAccountPre")}}
          <span
            class="highlight"
          >{{tradeAccount}}</span>
        </h4>
        <!-- <h4 class="desc-title">
          {{$t("iopenExt.guide.success.desc.fundAccountPre")}}
          <span
            class="highlight"
          >{{fundAccount}}</span>
        </h4>-->
        <template v-if="openType=='cn'">
          <p class="desc-tips">{{$t("iopenExt.guide.success.desc.cn[0]")}}</p>
          <p class="desc-tips">{{$t("iopenExt.guide.success.desc.cn[1]")}}</p>
          <p class="desc-tips">{{$t("iopenExt.guide.success.desc.cn[2]")}}</p>
        </template>
        <template v-if="openType=='hk'">
          <p class="desc-tips">{{$t("iopenExt.guide.success.desc.hk[0]")}}</p>
          <p class="desc-tips">{{$t("iopenExt.guide.success.desc.hk[1]")}}</p>
        </template>
        <p class="desc-tips">
