<template>
  <div class="op-com-signature">
    <header class="top">
      <p class="tip">{{$t('iopenExt.comSignature.title')}}</p>
    </header>
    <section class="content">
      ...
      <div class="sign-tips">
        <section class="tips" v-show="isShowTips">
          <p class="text">
            {{$t('iopenExt.comSignature.tipsContent')[0]}}
            <mark
              class="mark"
            >{{$t('iopenExt.comSignature.tipsContent')[1]}}</mark>
            {{$t('iopenExt.comSignature.tipsContent')[2]}}
            <mark
              class="mark"
            >{{$t('iopenExt.comSignature.tipsContent')[3]}}</mark>
            {{$t('iopenExt.comSignature.tipsContent')[4]}}{{userName}}
            <br />
            {{$t('iopenExt.comSignature.tipsContent')[5]}}
          </p>
    ...
    <footer ref="bottom" class="bottom">
      <section class="btn-wrapper">
        ...
        >{{$t('iopenExt.comSignature.btnClear')}}</cube-button>
        <cube-button
          class="btn"
          :disabled="!isDraw"
          @click="handleConfirmSign"
        >{{$t('iopenExt.comSignature.btnConfirm')}}</cube-button>
      </section>
    </footer>
  </div>
</template>
...
import SignaturePad from "@/modules/module-iopen-extension/assets/lib/signature_pad.js";
import { Signature } from "@/modules/module-iopen-extension/api/params-define";
