...
import {
  EMPTY_LABEL, FPS, CHECK, EDDA,
  EBANK, USD, HELP_URL, OTHER
  ...
  methods: {
    ...
    async fetchDepositBankList() {
      ...
      try {
        ...
        // 大陆
        bank_cn = await SecApi.bankListDeposit({ bankType: bank_type[1].value })
        // 其他地区
        bank_other = await SecApi.bankListDeposit({ bankType: bank_type[2].value })
      } catch (error) {
      }
    ...
    async getDepositAgainBank(data) {
      ...
      // 该银行卡类型下所有银行列表数据
      const temp_bankList = await SecApi.bankListDeposit({ bankType: data.bankType })
      ...
      if (wayKey === OTHER || FPS_CHECK_key.indexOf(wayKey) === -1) {
        ...
        temp_bankItem = temp_bankList.find(item => {
          ...
          const isSomeBank = (formatToDBC(item.name) === formatToDBC(data.remittanceBankName) || item.key === data.remittanceBankCorde)
          // return isSomeWay && isSomeBank
          return isSomeWay && isSomeBank && (item.key === data.remittanceBankCorde)
        })
      }
