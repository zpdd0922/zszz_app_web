<template>
  <div class="camera">
      ...
      <template v-if="getCompFile">
        ...
        <div v-if="showAction" class="file-action">
          ...
          <template v-if="isError">
            <span class="file-reload" @click="_clickReUpload">{{$t('iAccount.common.text_11')}}</span>
          </template>
          <!-- 上传示例 -->
          <template v-if="isExample">
            <span>{{ $t('iAccount.common.text_4') }}</span>
          </template>
        </div>
      </template>
      <!-- 未上传 - 显示上传按钮 -->
    ...
    <jf-action-sheet ref="jfActionSheet" :items="btns" @clickItem="_clickHandle"></jf-action-sheet>
  </div>
</template>
...
import {
  ...
  STATUS_SUCCESS,
  STATUS_ERROR,
} from "@/main/request/axios/upload";
import UploadTools, {
  BASE64,
  CAMERA_TYPE,
  openJsCameraJF,
...
const CAMERA_WAYS = [
  {
    // 相册
    title: "iAccount.common.text_5",
    type: "album",
  },
  {
    // 相机
    title: "iAccount.common.text_6",
    type: "camera",
  },
];
      ...
      const defaultOptions = {
        ...
        fail: (res, msg) => {
          ...
          tips.confirm({
            ...
            content: this.$t("iAccount.common.text_20"),
            onConfirm: () => {
