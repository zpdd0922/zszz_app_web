    ...
    <part-card :title="$t('iAccount.withdraw.request.text_3')">
      <ul class="detail-form">
        <li>
          ...
          <div class="form-filed">
            <div class="filed-item border-bottom-1px">
              <span class="txt">{{ withdrawBankInfo.receiptBankName | filterEmptyVal }}</span>
            </div>
          </div>
        </li>
        <!-- 选择其他银行 -->
        ...
        <li>
          ...
          <div class="form-filed">
            <div class="filed-item border-bottom-1px">
              <span v-if="withdrawBankHis" class="txt">{{model.withdrawBankAccount}}</span>
              <input
                v-else
...
export default {
  ...
  computed: {
    ...
    isDisabled() {
      ...
      const { withdrawBankNameOther, withdrawBankAccountAgain, swiftCode, withdrawMoney, currency, withdrawBankAccount } = this.model
      const isRequired = [currency, withdrawBankAccount].every(item => String(item).length) && receiptBankName && Number(withdrawMoney)
      const isRequired_cn = [swiftCode].every(item => item.length)
      const temp = this.isBank_CN && this.isBank_other ? (isRequired && isRequired_cn) : isRequired
      // 选择其他银行
      const result = this.isShowOther ? temp && withdrawBankNameOther.length : temp
      const isEmpty = this.withdrawBankHis ? result : withdrawBankAccountAgain.length && result
      return !isEmpty
    },
    getCurrency() {
  ...
  methods: {
    _updateInfo() {
      ...
      if (this.withdrawBankHis) {
        ...
        SecApi.withdrawBank({ extMethod: this.withdrawBankInfo.bankType }).then(res => {
