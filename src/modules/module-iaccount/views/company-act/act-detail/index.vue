<template>
  <div class="wrap">
    <div class="content">
      <div class="item">
        <p class="label">{{$t('iAccount.company_act.label.text_1')}}</p>
        <p class="text">{{bourseName}}</p>
      </div>
      <div class="item pt">
        <p class="label">{{$t('iAccount.company_act.label.text_2')}}</p>
        <p class="text">{{companyName}}</p>
      </div>
      <div class="item pt" v-if="actionName">
        <p class="label">{{$t('iAccount.company_act.label.text_3')}}</p>
        <p class="text">{{actionName}}</p>
      </div>
      <!-- 部分股票 部分现金 -->
      <template v-if="OSO">
        <div class="item pt">
          <p class="label">{{$t('iAccount.company_act.label.text_5')}}</p>
      ...
      <template v-if="OSE">
        <div class="item pt">
          <p class="label">{{$t('iAccount.company_act.label.text_7')}}</p>
          <p class="text">{{renderStockAccount}}</p>
        </div>
      </template>
      <template v-if="RS">
        <div class="item pt">
          <p class="label">{{$t('iAccount.company_act.label.text_8')}}</p>
          <p class="text">{{renderStockAccount}}</p>
        </div>
      </template>
      <template v-if="RSOhter">
        <div class="item pt">
          <p class="label">{{$t('iAccount.company_act.label.text_9')}}</p>
          <p class="text">{{renderAdditionalStkAmount}}</p>
        </div>
      </template>
      <template v-if="CO">
        <div class="item pt">
          <p class="label">{{$t('iAccount.company_act.label.text_30')}}</p>
          <p class="text">{{renderStockAccount}}</p>
        </div>
      </template>
      <template v-if="OTHER">
        <div class="item pt">
          <p class="label">{{$t('iAccount.company_act.label.text_4')}}</p>
          <p class="text">{{renderStockAccount}}</p>
        </div>
      </template>
      <div class="item pt">
        <p class="label">{{$t('iAccount.company_act.label.text_12')}}</p>
        <p class="text">{{renderRemark}}</p>
      </div>
      <div class="item pt">
        <p class="label">{{$t('iAccount.company_act.label.text_31')}}</p>
        <p class="text">{{formatTime}}</p>
      </div>
    </div>
    <div class="notice">
      <p>{{$t('iAccount.company_act.notice.text_1')}}</p>
      <p>{{$t('iAccount.company_act.notice.text_2')}}</p>
    </div>
    <div class="btn">
      <cube-button primary  @click="submitHandle">{{ $t('iAccount.company_act.label.text_32') }}</cube-button>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    bourseName() {
      ...
      const data = this.$t('iAccount.define.BOURSE_OPTIONS').filter(val => val.value === exchangeType)
      return data.length && data[0].text
    },
    companyName() {
      const { actionNameType } = this.$route.params.data
      const data = this.$t('iAccount.define.COMPANY_NAME_OPTIONS').filter(val => val.value === actionNameType)
      return data.length && data[0].text
    },
    actionName() {
      const list = [
        ...this.$t('iAccount.define.SCRIP_OPTIONS'),
        ...this.$t('iAccount.define.SHARE_EXCHANGE_OPTIONS'),
        ...this.$t('iAccount.define.RIGHTS_SUBSCRIPTION_OPTIONS'),
        ...this.$t('iAccount.define.OPEN_OFFER_SUBSCRIPTION_OPTIONS'),
        ...this.$t('iAccount.define.CASH_OFFER'),
        ...this.$t('iAccount.define.WARRANTS_CONVERSION_OPTIONS'),
        ...this.$t('iAccount.define.PREFERENTIAL_OFFER')
      ]
