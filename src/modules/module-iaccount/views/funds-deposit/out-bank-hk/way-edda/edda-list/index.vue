<template>
  <div class="edda-list">
    <part-list
      :title="$t('iAccount.deposit.edda.edda_list.title')"
      :isLink="false"
      :list="eddaCard"
      ...
      <template v-slot:desc="slotProps">
        <div class="item">
          ...
          <div v-if="slotProps.item.eddaState === 2" class="del" @click.stop="_del(slotProps.item)"><span>{{$t('iAccount.common.text_23')}}</span></div>
    ...
    <div class="btn" @click="_clickBtn">
      ...
      <span class="txt">{{$t('iAccount.deposit.edda.edda_list.text_1')}}</span>
    </div>
...
export default {
  ...
  methods: {
    ...
    _del(item) {
      tips.confirm({
        title: this.$t('iAccount.deposit.edda.edda_list.text_2'),
        content: this.$t('iAccount.deposit.edda.edda_list.text_3'),
        onConfirm: () => {
          const params = {
          ...
          SecApi.eddaBankDelete(params).then(res => {
            this.fetchBankList()
            tips.toast({ txt: this.$t('iAccount.deposit.edda.edda_list.text_4') })
          })
        }
      })
    },
