<template>
  <div class="bind">
    <jf-wrap
      ...
      :nextBtnFixed="false"
      :btnText="$t('deposit.edda.bind.text_14')"
    ...
    >
      ...
      <div class="content">
          ...
          <ul>
            ...
            <li>
              <div class="form-label">
                <span>{{$t('deposit.edda.bind.text_1')}}</span>
              </div>
            ...
            <li>
              <div class="form-label">
                <span>{{$t('deposit.edda.bind.text_2')}}</span>
              </div>
              <div class="form-filed">
                <span>{{model.depositAccountName}}</span>
              </div>
              <p class="tips">{{$t('deposit.edda.bind.text_4')}}</p>
            </li>
            <!-- 银行账户类型 -->
            <li>
              <div class="form-label">
                <span>{{$t('deposit.edda.bind.text_5')}}</span>
                <jf-icon name="tips_gray" @click.native.stop="_showTips()"></jf-icon>
              </div>
              <div class="form-filed">
                <cube-select
                  v-model="model.depositAccountType"
                  :options="$t('define.DEPOSIT_ACCOUNT_TYPE')">
                </cube-select>
              </div>
            </li>
            <!-- 银行账号 -->
            <li>
              <div class="form-label">
                <span>{{$t('deposit.edda.bind.text_7')}}</span>
              </div>
              <div class="form-filed">
                <div class="filed-flex">
                  ...
                  <input
                    type="tel"
                    :placeholder="$t('deposit.edda.bind.text_8')"
            ...
            <li>
              <div class="form-label">
                <span>{{$t('deposit.edda.bind.text_9')}}</span>
              </div>
              <div class="form-filed">
                <cube-select
                  v-model="model.bankIdKind"
                  :options="$t('define.BANK_ID_KIND')"
                  @change="handleKind">
                </cube-select>
              </div>
            </li>
            ...
            <li>
              <div class="form-label">
                <span>{{$t('deposit.edda.bind.text_10')}}</span>
              </div>
              <div class="form-filed">
                <div class="filed-item">
                  ...
                  <template v-else>
                    <input
                      type="text"
                      :placeholder="$t('deposit.edda.bind.text_11')"
        ...
        <div class="agreement">
          ...
          <span class="act" @click="_changeAgreement">{{$t('deposit.edda.bind.text_12')}}</span>
          <span class="agr" @click="goAgreement">{{$t('deposit.edda.bind.text_13')}}</span>
        </div>
      </div>
    </jf-wrap>
    <div class="warn">
      <p>{{$t('deposit.edda.bind.tips.text_1')}}</p>
      <p>{{$t('deposit.edda.bind.tips.text_2')}}</p>
      <p>{{$t('deposit.edda.bind.tips.text_3')}}</p>
    </div>
    <!-- 弹框 -->
    <div v-show="isShowEddaTips">
      <jf-dialog
        ...
        <template v-slot:default>
          <div class="dialog">
            <p class="title">{{$t('deposit.edda.bind.text_5')}}</p>
            <div>
              <p>{{$t('deposit.edda.bind.tips.text_4')}}</p>
              <p>{{$t('deposit.edda.bind.tips.text_5')}}</p>
            </div>
            <div>
              <p>{{$t('deposit.edda.bind.tips.text_6')}}</p>
              <p>{{$t('deposit.edda.bind.tips.text_7')}}</p>
            </div>
          </div>
        </template>
      </jf-dialog>
    ...
    <div v-show="isShowAgrTips">
      <jf-dialog
        ...
        <template v-slot:default>
          <div class="agr-list" v-html="$t('deposit.edda.bind.tips.text_10')"></div>
        </template>
      </jf-dialog>
    </div>
  </div>
...
const GGREEMENT_TXT = [
  {
    title: '',
    list: [
      { txt: 'deposit.edda.bind.tips.text_10' }
    ]
  },
  {
    title: 'deposit.edda.bind.tips.text_11',
    list: [
      { txt: 'deposit.edda.bind.tips.text_12' }
    ]
  },
  {
    title: 'deposit.edda.bind.tips.text_13',
    list: [
      { txt: 'deposit.edda.bind.tips.text_14' }
    ]
  }
]
...
export default {
  ...
  methods: {
    updateData() {
      ...
      this.model.depositAccountName = this.accInfo.clientNameEn
    ...
    handleBefore() {
      ...
      return new Promise((resolve, reject) => {
        if (!validate.isNumber(depositAccount)) {
          const msg = this.$t('deposit.edda.bind.tips.text_15')
        ...
        case 1:
          if (!validate.isIdCard(bankIdNo)) {
            const msg = this.$t('deposit.edda.bind.tips.text_9')
            this.errorStatus = true
            this.errorMsg = msg
            return reject(msg)
          }
